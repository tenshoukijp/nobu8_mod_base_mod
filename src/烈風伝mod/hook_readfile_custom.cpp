...
struct KAMON_PICLINE {
    BYTE at[KAMON_PIC_WIDTH];
};
struct KAMON_PICTURE {
    KAMON_PICLINE line[KAMON_PIC_HIGHT];
};


extern int nTargetKamonCGID;
BOOL Hook_ReadFileCustom_KamonPic(
    ...
    if (nTargetKamonCGID < 0) {
        return FALSE;
    }

    std::unique_ptr<KAMON_PICTURE> picKamonCGOrigin(new KAMON_PICTURE);
    std::unique_ptr<KAMON_PICTURE> picKamonCGSidePp(new KAMON_PICTURE);

    char filenameBuf[512] = "";
    std::string jsOverridePath = callJSModRequestKamonPicID(nTargetKamonCGID);
    if (jsOverridePath != "") {
        strcpy_s(filenameBuf, jsOverridePath.c_str());
    }
    else {
        sprintf_s(filenameBuf, "OVERRIDE\\KAMONCG\\%03d.bmp", nTargetKamonCGID);
    }
    ...
    // 元の画像をコピー
    if (nNumberOfBytesToRead == buffer.size()) {
        memcpy(picKamonCGOrigin.get(), buffer.data(), buffer.size());

        // 上下反転したものを picfileOrigin→picflipSidePp にコピー
        for (int i = 0; i < KAMON_PIC_HIGHT; i++) {
            memcpy(&(picKamonCGSidePp->line[KAMON_PIC_HIGHT - 1 - i]), &(picKamonCGOrigin->line[i]), KAMON_PIC_WIDTH);
        }

        memcpy(lpBuffer, picKamonCGSidePp.get(), (KAMON_PIC_WIDTH * KAMON_PIC_HIGHT));
    }
